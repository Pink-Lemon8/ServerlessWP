...
?>
<div id="orders" class="mt-20" style="display:none;">
    <div class="mx-auto">
          ...
          <div class="space-y-1 md:flex md:items-baseline md:space-x-4 md:space-y-0">
            ...
            <div class="py-6 sm:flex">
              ...
              <div class="mt-6 space-y-4 sm:ml-6 sm:mt-0 sm:w-40 sm:flex-none">
                <button type="button" onclick="window.open('<?= get_permalink($product_post->ID) ?>');" class="flex w-full items-center justify-center rounded-md border border-transparent bg-[<?= MAIN_COLOR ?>] px-2.5 py-2 text-sm font-medium text-white shadow-sm hover:bg-[<?= MAIN_COLOR_HOVER ?>] focus:outline-none focus:ring-2 focus:ring-[<?= MAIN_COLOR_FOCUS ?>] focus:ring-offset-2 sm:w-full sm:flex-grow-0">Buy again</button>
              </div>
            </div>
            <?php endforeach; ?>
