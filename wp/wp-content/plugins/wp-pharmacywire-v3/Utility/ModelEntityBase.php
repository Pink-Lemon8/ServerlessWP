<?php
class Utility_ModelEntityBase implements ArrayAccess, JsonSerializable
{
	...
	protected function __toXml(array $arrAttributes = array(), $rootName = 'item', $addOpenTag = false, $addCdata = true)
	{
		...
		foreach ($arrData as $fieldName => $fieldValue) {
			if ($addCdata === true) {
				...
				$fieldValue = $this->__xmlEntities($fieldValue);
			}
	...
	protected function __xmlEntities($value) {
		if (empty($value)) {
			$value = $this;
		}
		$value = (string)$value;
	...
	protected function _camelize($name)
	{
		...
		$destSep = '';
		$srcSep = '_';
		return str_replace(' ', $destSep, ucwords(str_replace($srcSep, ' ', $name)));
	}
